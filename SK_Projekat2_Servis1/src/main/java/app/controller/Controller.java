...
public class Controller {
	...
	public ResponseEntity<String> addCard(@RequestBody CreditCardForm creditCardForm,@RequestHeader(value = HEADER_STRING) String token) {
		...
		try {
			...
			if (user == null && adminRepo.findByUsername(email) !=null) {
				return new ResponseEntity<>("Only for Users", HttpStatus.BAD_REQUEST);
			}
			// iscitavamo entitet iz registracione forme
			CreditCard card = new CreditCard(user.getIme(),user.getPrezime(), creditCardForm.getBrojKartice(), creditCardForm.getSigurnosniBroj(), user.getId());
	...
	public ResponseEntity<String> changeUser(@RequestBody ChangeForm changeForm, @RequestHeader(value = HEADER_STRING) String token) {
		try {
			...
			userRepo.saveAndFlush(user);	        
			return new ResponseEntity<>(user.getMilje() + " " + user.getRank(), HttpStatus.ACCEPTED);
		} catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
		}
